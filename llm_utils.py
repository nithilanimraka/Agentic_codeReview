...
class FinalReview(BaseModel):
        ...
        start_line: int = Field(description="The starting line number in the file (REQUIRED)")
                                    
        end_line: int = Field(description="The ending line number in the file (REQUIRED)")
...
def aggregator(state: State):
    ...
    for review in error_issues:
        ...
        error_lines.append(f"Lines: {review.start_line}-{review.end_line}")
    ...
    for review in security_issues:
        ...
        security_lines.append(f"Lines: {review.start_line}-{review.end_line}")
    ...
    for review in performance_issues:
        ...
        performance_lines.append(f"Lines: {review.start_line}-{review.end_line}")
    ...
    for review in quality_issues:
        ...
        quality_lines.append(f"Lines: {review.start_line}-{review.end_line}")
    ...
    for review in other_issues:
        ...
        other_lines.append(f"Lines: {review.start_line}-{review.end_line}")
...
# Invoke
state = parallel_workflow.invoke({"PR_data": pr_input})
...
def final_review(pr_data:str ,final_issues:str):
...
a=final_review(pr_input,final_issues)
print(a)
